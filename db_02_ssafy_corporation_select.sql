...
/*
    ...
    Ctrl + enter : 해당 줄 실행
    Ctrl + B : 자동정렬
    sql 문도 마지막엔 ;을 찍어서 문장을 마무리 해야 한다!
*/
...
SELECT ename, empno, comm, sal, sal+comm AS "커미션 포함 급여", sal+ifnull(comm, 0) AS"커미션 포함 급여2"
FROM emp;
...
SELECT COUNT(*) AS 사원수, SUM(sal) 급여총액, AVG(sal) 평균급여, MAX(sal) 최고급여, MIN(sal) 최소급여
FROM emp;
...
SELECT deptno, count(*), round(AVG(sal +IFNULL(comm,0)),2) AS '평균 급여(커미션 포함)'
FROM emp
GROUP BY deptno
